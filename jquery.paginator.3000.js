...
;(function($){
	...
	$.fn.paginator = function (s){			
		var options = {
			...
			pagesSpan   : 10,
			pageCurrent : 50,
			baseUrl     : document.location+'&page=%number%',
			buildCounter : function (page) {
				return page;	
			},
			clickHandler: null,
			returnOrder : false,
		...
		function getLink(page){
			return options.baseUrl.replace(/%number%/i, options.buildCounter(page));
		} 		
		...
		function bindLink(el){
			if ($.isFunction(options.clickHandler)){
				$(el).find('a').bind('click', function (){
					return options.clickHandler(options.buildCounter($(this).text()));
				});  
			}
		}	
